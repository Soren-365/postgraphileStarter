...
import { psqltestMutation } from "../../functions/fetch/graphql/psqltestMutation";
...
export function* psqlMutation({
  ...
  try {
    console.log(
      "COMMCONTROL, input: mutationType, tableKey, variables:",
      mutationType,
      tableKey,
      variables
    );
    ...
    console.log("response from psqlMutation( query, atAgency)", response);
    yield put({
...
export function* psqlUpdateMutation({
  ...
  try {
    console.log(
      "COMMCONTROL, input: tableKey, variables:",
      tableKey,
      variables
    );
...
export function* psqlTestMutation({ tableKey, variables }: TestMutationType) {
  try {
    console.log(
      "COMMCONTROL, input: tableKey, variables:",
      tableKey,
    ...
    const args = { tableKey, variables };
    const response: any = yield call(psqltestMutation, args);
    yield put({
      type: API.MUTATE_PGDB_SUCCESS,
      message: `success In commHelper psqlTestMutation(), response: ${response.data}`,
    });
    return response.data;
  } catch (error) {
    yield put({
      type: API.MUTATE_PGDB_ERROR,
      message: `error In commHelper psqlTestMutation() ${error}`,
    });
  }
}
