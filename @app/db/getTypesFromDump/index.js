...
while (placementOfNextEnum !== -1) {
  ...
  const enumName = enumNamePre.slice(enumNamePre.indexOf('.'), enumNamePre.length)
...
while (tableName !== "") {
  ...
  tableNamePre = tableCutOut.slice(1, indexOfnextSpace + 1)
  ...
  while (indexOfnextComma !== -1) {
    ...
    if (firstRow) {
      refSlice = restOfTable.slice(indexOfgetFirstPureParanthesis !== -1 ? (indexOfgetFirstPureParanthesis + 1) : 0, ((indexOfnextComma !== -1) ? (indexOfnextComma + 1) : 0)).trim()
      console.log("Row Array Empty! special case, removing \"(\"")

    } else {
      refSlice = restOfTable.slice(0, indexOfnextComma !== -1 ? (indexOfnextComma + 1) : 0).trim()
    ...
    if (indexOfnextLetterFromRef === indexOfgetNextApostropheFromRef) {
      ...
      indexOfendOfRowName = restOfTable.slice(indexOfstartOfRowName, restOfTable.length).trim().indexOf(" ")
    }
    ...
    if (!(indexOfnextComma === -1) && (multipleKeyDetect === false) && (multipleKeyDetectOld === false)) {
      if ((indexOfgetFirstApostrophe !== indexOfgetFirstParanthesisApostrophe + 1) && (indexOfgetSecondParanthesis !== indexOfnextComma + 3)) {
        ...
        console.log("rowName", '|', rowName, '|', rowName.length)
        let indexOfnextLine = restOfTable.slice(indexOfnextComma + 1, restOfTable.length).indexOf('\n')
        ...
        primaryKeyDetect = Boolean(lineSlice2.toLowerCase().search("primary key") !== -1);
      ...
      if (rowName) {
        ...
        if ((rowTypeRaw.toLowerCase() === 'key')) {
          ...
          console.log("Foreign key adding, ignoring array entry function")
        }
        else {
    ...
    // // console.log("restOfTable ", restOfTable)
      ...
      // restOfTable = restOfTable.slice(indexOfnextComma + 1, restOfTable.length)
      // console.log("restofTable", restOfTable)
      // indexOfnextComma = restOfTable.indexOf(",")
...
for (i in tableObjectArray) {
  ...
  //console.log("tableObjectArray[i]", tableObjectArray[i])
